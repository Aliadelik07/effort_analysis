...
dfa_condition_1 <- dfa[dfa$n_back == 1, c("sub", "ms_norm")]
dfa_condition_3 <- dfa[dfa$n_back == 3, c("sub", "ms_norm")]
...
pwr.t.test(d = cohen_d, n = nrow(merged_dfa), sig.level = 0.05, type = "paired", alternative = "two.sided")

pwr.t.test(d = cohen_d, power = 0.80, sig.level = 0.05, type = "paired", alternative = "two.sided")

power_analysis <- pwr.t.test(d = cohen_d, power = 0.80, sig.level = 0.05, type = "paired", alternative = "two.sided")
...
ggplot(plot_data, aes(x = value, color = group)) +
  ...
  geom_errorbarh(aes(xmin = mean(merged_dfa$treatment, na.rm = TRUE) - sd(merged_dfa$treatment, na.rm = TRUE),
                     ...
                     y = 0.04), height = 0.002, color = "blue", linetype = "dashed") +
  # Add p-value, t-statistic, degrees of freedom, and Cohen's d as text
  annotate("text", x = mean(merged_dfa$baseline, na.rm = TRUE), y = 2.1, 
           label = paste0("t: ", round(t_test$statistic, 2), 
                          ", df: ", round(t_test$parameter, 1), 
                          ", p-value: ", round(t_test$p.value, 4),
                          if(!is.na(cohen_d)) paste0(", Cohen's d: ", round(cohen_d, 2)) else ""),
           size = 4, color = "black") +
  labs(title = "Density Plot of Baseline vs Treatment",
       x = "Values", y = "Probability", color = "Group") +  # Legend title
  scale_color_manual(values = c("Baseline" = "red", "Treatment" = "blue")) +  # Match colors with lines
  theme_minimal()
